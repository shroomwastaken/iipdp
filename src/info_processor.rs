...
pub fn dump_file(file_path: &String, demo: Demo) {
    ...
    file.write_all("Generated by IIPDP v0.2.7\n".as_bytes());
...
pub fn dump_flattened_classes(file_path: &String, prop_lookup: Vec<(ServerClass, Vec<FlattenedProp>)>) {
    let mut file = fs::File::create(file_path.trim_end_matches(".dem").to_owned() + "-prop_dump.txt").unwrap_or_else( |err| {
        println!("Something went wrong when trying to create the file: {}", err);
        io::stdin().read_line(&mut String::new()).unwrap();
        exit(1);
    });
    
    println!("File created at: {}\n", file_path.trim_end_matches(".dem").to_owned() + "-prop_dump.txt");
    ...
    for bundle in prop_lookup {
        ...
        for prop in bundle.1 {
            ...
            base_str.replace_range(0..prop_type_len, prop.prop_info.send_prop_type.into());
            base_str.replace_range(10..10 + prop.name.len(), &prop.name);
            file.write_fmt(format_args!("\n\t{}", base_str));
        }
    }
}